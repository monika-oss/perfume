...
// Custom detailed inline SVGs matching the exact drawings in the poster
const IdealForIcon = ({ label }) => {
  ...
  if (name.includes("detergent powder")) {
    return (
      <motion.svg viewBox="0 0 64 64" fill="none" stroke={strokeColor} strokeWidth={strokeWidth} strokeLinecap="round" strokeLinejoin="round" className="w-14 h-14">
  ...
  return (
    <section id="ideal-for" className="py-20 scroll-mt-6 md:scroll-mt-12 bg-white relative">
      <div className="container mx-auto px-4">
        ...
        {/* Border Box exactly matching the poster's visual frame layout */}
        {/* Border Box exactly matching the poster's visual frame layout */}
        <motion.div 
          initial="hidden"
          whileInView="visible"
          ...
          variants={{
            ...
            visible: { 
              opacity: 1, 
              scale: 1, 
              transition: { type: "spring", bounce: 0.4, duration: 0.8, staggerChildren: 0.1 } 
            }
          }}
          className="relative border-2 border-brand-purple/20 rounded-3xl pt-12 pb-8 px-4 sm:px-6 shadow-sm"
        >
          
          {/* Header embedded on the top border exactly like the poster */}
          <motion.div 
            animate={{ scale: [1, 1.05, 1], y: ["-50%", "-60%", "-50%"] }}
            transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-0 left-1/2 -translate-x-1/2 bg-brand-purple-dark text-white px-8 py-2 rounded-md shadow-md font-black text-sm md:text-base tracking-widest uppercase border border-purple-300"
          >
            IDEAL FOR
          </motion.div>

          {/* Grid layout with dividers */}
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-y-8 divide-x-0 sm:divide-x lg:divide-x divide-purple-100 mt-4">
            {items.map((item, index) => (
              <motion.div
                ...
                className="flex flex-col items-center gap-4 text-center px-2 cursor-pointer group"
              >
                {/* SVG Icon matching the poster details */}
                <motion.div 
                  initial="hidden"
                  whileInView="visible"
